...
Java中的8中原生数据类型，都有各自对应的Buffer类型，如IntBuffer.LongBuffer,ShortBuffer...
...
与java.io 中流（Stream）不同的是，channel是双向的，一个流只可能是Inputstream或者OutputStream,Channel打开后则可以进行读取、写入或者读写。
